...
def parse_results(meta_path, annotation_path):
...
def evaluate(tool):
    ...
    for transform in ["naming", "expression", "statement"]:
        ...
        transform_results = parse_results(open(f"data/meta/{transform}.json", 'r'), f"data/plausible_patches/{transform}-{tool}.xlsx")
        ...
        print("Total changes: " + str(len(transform_changes)))
    ...
    evaluation_metrics(original_results, all_bugs)
    print("== Performance on Transformed Data ==")
    calculate_metrics(all_plausible, all_correct, all_bugs)
    
    print("== Unique Change Rate ==")
    unique_change_rate(all_positive_changes, all_negative_changes, all_bugs)
    
    print("== Normalized Change Rate ==")
    normalized_change_rate(all_positive_changes, all_negative_changes, all_bugs)
    f.close()
    return all_changes
...
print("----------------------------------------------------------------------")
print("====> Robustness of NPR against semantic-preserving transformations")
changes = {
...
print("----------------------------------------------------------------------")
print("==> [RQ2.1 - Section 4.2.1] Impact of Unnatural Code Transformations on Robustness Testing")
...
for tool in changes:  
    print("==> There are {} changes for {} including: ".format(len(changes[tool]), tool))
    print(changes[tool])
    
    for cat in change_categories:
        total = 0
        for change in changes[tool]:
            if change in naturalness_evaluation[cat]:
                change_categories[cat].add(change)
                total += 1
        naturalness_ratio[tool].append(total/len(changes[tool]))
        print("{} changes are {} ({}%)".format(total, cat, round(total/len(changes[tool]) * 100, 2)))
   
print("==> Change Categories: ")     
for cat in change_categories:  
    print("There are {} changes for {} including: ".format(len(change_categories[cat]), cat))
    print(change_categories[cat])

for tool in naturalness_ratio:
    assert sum(naturalness_ratio[tool]) == 1

print(naturalness_ratio)
values = np.array(list(naturalness_ratio.values()))
total = values.sum(axis=0)/5
print(values.shape)
print(total)
values = np.concatenate((values, total.reshape(1, 4)), axis=0)
values = np.round(values * 100, 1)
categories = ['SequenceR', 'Recorder', 'AlphaRepair', 'SelfAPR', 'RewardRepair', 'Overall']


fig, ax = plt.subplots(figsize=(10, 3))
column_width = 0.8

bars1 = ax.bar(categories, values[:, 0], label='Unnatural', edgecolor="black", color="tab:orange", width=column_width)
ax.bar_label(bars1, label_type='center')

bars2 = ax.bar(categories, values[:, 1] + values[:, 2], bottom=values[:, 0], label='Likely Natural/Unnatural', edgecolor="black", color="tab:gray", width=column_width)
ax.bar_label(bars2, label_type='center')
bars3 = ax.bar(categories, values[:, 3], bottom=values[:, 0] + values[:, 1] + values[:, 2], label='Natural', edgecolor="black", color="tab:green", width=column_width)
ax.bar_label(bars3, label_type='center')
ax.legend(title='Categories', bbox_to_anchor=(1.05, 1), loc='upper left')

# Add labels and a legend
# plt.xlabel('Transformation Level', fontsize=14)
...
print("=> Please see results in data/figures/rq2_naturalness.png")
plt.savefig("data/figures/rq2_naturalness.png", dpi=300)
